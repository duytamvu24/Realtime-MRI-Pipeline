...
def start_analysis(params, log_widget):
    ...
    log("Folgende Schichten wurden im Dicom-Pfad gefunden:")
    ...
    print("Schichten und Bilder wurden sortiert!")
    ...
    log("Die Log-Zeit wird eingelesen:")
    ...
    log("Spirodaten werden mit Bildern korreliert, indem der Zeitstempel genutzt wird:")
    ...
    log(f"Überschreiben der Instance Number für die Sortierung!")
    count = 0
    ...
    log("Reinschreiben der Spiroinformationen in die Dicom-Tags:")
    k = []
    for i in scans_sort:
        l = []
    ...
    log("Vorbereitung fürs Binning: Jedes Bild wird in EKG-Klassen und in Inspiration oder Expiration aufgeteilt!")
    count = 0
    scans_sort_by_ecg = {}
    for i in scans_sort:
    ...
    log(f'{count} images processed')
    log("Exportieren der Daten: Nur Bilder in Endexspiration mit dem kleinsten Volumen von jeder EKG-Klasse. Falls es keinen gibt, wird dieser aufgefüllt.")
...
def ask_user_inputs():
    root = tk.Tk()
    root.title("Parameter & Datei-Auswahl")

    results = {}

    def choose_file(var, filetypes=(("Alle Dateien", "*.*"),)):
    ...
    row = 0
    ttk.Label(frame, text="Main-Pfad:").grid(row=row, column=0, sticky="w")
    ttk.Entry(frame, textvariable=main_path, width=50).grid(row=row, column=1)
    ttk.Button(frame, text="Wählen", command=lambda: choose_dir(main_path)).grid(row=row, column=2)
    row += 1

    ttk.Label(frame, text="DICOM-Pfad:").grid(row=row, column=0, sticky="w")
    ttk.Entry(frame, textvariable=dicom_path, width=50).grid(row=row, column=1)
    ttk.Button(frame, text="Wählen", command=lambda: choose_dir(dicom_path)).grid(row=row, column=2)
    row += 1

    ttk.Label(frame, text="Spirodatei:").grid(row=row, column=0, sticky="w")
    ttk.Entry(frame, textvariable=spiro_file, width=50).grid(row=row, column=1)
    ttk.Button(frame, text="Wählen", command=lambda: choose_file(spiro_file)).grid(row=row, column=2)
    row += 1
    ...
    ttk.Button(frame, text="Wählen", command=lambda: choose_file(timestamp_npy, (("NumPy Datei", "*.npy"),))).grid(row=row, column=2)
    row += 1
    ...
    ttk.Button(frame, text="Wählen", command=lambda: choose_file(indizes_npy, (("NumPy Datei", "*.npy"),))).grid(row=row, column=2)
    row += 1
    ...
    ttk.Label(frame, text="manuelle verschiebung in Zeitschritten:").grid(row=row, column=0, sticky="w")
    ttk.Entry(frame, textvariable=manuelle_verschiebung, width=20).grid(row=row, column=1, sticky="w")
    row += 1

    ttk.Label(frame, text="Anzahl EKG-Phasen:").grid(row=row, column=0, sticky="w")
    ...
    ttk.Button(frame, text="START Analyse", command=start).grid(row=row, column=0, columnspan=3, pady=10)
