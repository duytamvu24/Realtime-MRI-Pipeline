...
def choose_files():
    def select_ecg():
        filename = filedialog.askopenfilename(title="EKG-Datei wählen", filetypes=[("Alle Dateien", "*.*")])
        if filename:
            entry_ecg.delete(0, tk.END)
            entry_ecg.insert(0, filename)

    def select_spiro():
        filename = filedialog.askopenfilename(title="Spiro-Datei wählen", filetypes=[("Alle Dateien", "*.*")])
        if filename:
    ...
    def start_program():
        ...
        if not ecg_file or not spiro_file:
            tk.messagebox.showerror("Fehler", "Bitte beide Dateien auswählen!")
            return
        root.destroy()
    ...
    # EKG-Kanal
    ttk.Label(frm, text="EKG-Datei:").grid(row=0, column=0, sticky="w")
    entry_ecg = ttk.Entry(frm, width=50)
    entry_ecg.grid(row=0, column=1, padx=5)
    ttk.Button(frm, text="Wählen", command=select_ecg).grid(row=0, column=2)

    # EKG-Kanal
    ttk.Label(frm, text="EKG-Kanal:").grid(row=1, column=0, sticky="w")
    ...
    # Spiro-Datei
    ttk.Label(frm, text="Spiro-Datei:").grid(row=2, column=0, sticky="w")
    entry_spiro = ttk.Entry(frm, width=50)
    entry_spiro.grid(row=2, column=1, padx=5)
    ttk.Button(frm, text="Wählen", command=select_spiro).grid(row=2, column=2)
...
def launch_main_window(filename, filename_spiro, ecg_kanal):
    ...
    x_datas = [
        np.arange(len(signals[0])) * 0.0025,   # ECG Zeitachse
        np.arange(len(signals[1])) * 0.008     # Spiro Zeitachse
    ]
    ...
    def onclick(event):
        if point_mode.get():
            if event.xdata is not None and event.ydata is not None:
                ...
                print(f"Punkt gesetzt bei Signal {current_signal_index+1}: ({nearest_x}, {nearest_y:.2f})")
                update_plot()
    ...
    def save_point_and_next():
        ...
        if time_stamp:
            ...
            if current_signal_index < len(signals):
                ...
                print("Alle Signale durchlaufen!")
                print("Gespeicherte Punkte:", clicked_points)
                final_correlation()
    ...
    def final_correlation():
        print("\n--- Starte Korrelations-Auswertung ---")
        ...
        result_frame = ttk.LabelFrame(root, text="Übereinandergelegte Signale")
        result_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
    
        fig2, ax2 = plt.subplots(figsize=(10, 4))
        line_ecg, = ax2.plot(time_ecg, mod_ecg_data, label="ECG skaliert")
        line_spiro, = ax2.plot(time_spiro, spiro_from_timestamp, label="Spiro")
        ax2.set_xlabel("Zeit [s]")
        ax2.set_ylabel("Amplitude")
        ax2.set_title("Übereinandergelegte Signale")
        ax2.legend()
        ax2.grid(True)
        ...
        def save_correlated_data():
            ...
            print("Daten gespeichert: ecg.npy, spiro.npy, indizes.npy")
        ...
        save_data_btn = ttk.Button(result_frame, text="Daten speichern", command=save_correlated_data)
    ...
    toggle_btn = ttk.Checkbutton(button_frame, text="Punkt setzen aktivieren", variable=point_mode)
    toggle_btn.pack(side=tk.LEFT, padx=5)

    save_btn = ttk.Button(button_frame, text="Punkt speichern & nächster Graph", command=save_point_and_next)
