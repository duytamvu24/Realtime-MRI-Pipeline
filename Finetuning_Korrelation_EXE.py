...
def select_dicom_folder():
    ...
    if folder:
        ...
        dicom_label.config(text=f"Selected: {folder}")

def select_spiro():
    # Select Spiro-log file
    file = filedialog.askopenfilename(
        title="Select Spiro-File (...raw.log)",
        filetypes=[("Log-File", "*.log")]
    )
    if file:
        params["spiro_file"] = file
        spiro_label.config(text=f"Selected: {file}")
...
def select_indizes():
    ...
    if file:
        ...
        indizes_label.config(text=f"Selected: {file}")

def select_zeitstempel():
    # select timestamps 
    file = filedialog.askopenfilename(
        title="Select timestamp-file",
        filetypes=[("NumPy-File", "*.npy")]
    )
    if file:
        params["zeitstempel_file"] = file
        zeitstempel_label.config(text=f"Selected: {file}")

def select_bauchgurt():
    # select respiratory bellow file
    file = filedialog.askopenfilename(
        title="Select respiratory-bellow-file",
        filetypes=[("Resp-File", "*.resp")]
    )
    if file:
        params["bauchgurt_file"] = file
        bauchgurt_label.config(text=f"Selected: {file}")
...
def start_analysis():
    ...
    try:
        ...
        messagebox.showerror("Error", "Please select desired slice number!")
        return
    ...
    slice_slider = tk.Scale(plot_frame, from_=0, to=max_slice, orient=tk.HORIZONTAL,
                            label="Select slice", command=change_slice, length=400)
    slice_slider.set(slice_init)
    slice_slider.pack(pady=5)

    # Click to create the ROI
    btn_roi = tk.Button(plot_frame, text="Select ROI", command=reset_roi)
    btn_roi.pack(pady=5)

    btn_finetuning = tk.Button(plot_frame, text = "Save signal intensivity curve!", command = finetuning)
...
def finetuning():
    ...
    if shift_entry is not None:
        ...
        if val.strip() != "":
            try:
                ...
                print("Invalid value, please type in a number.")
                shift = 0  
    ...
    ax.plot(timescale_images,vol, label='Volumetry from SI-curve',)
...
def plot_everything():
    ...
    if shift_entry is not None:
        ...
        if val.strip() != "":
            try:
                ...
                print("Invalid value. Type in number")
                shift = 0
    ...
    line1, = ax.plot(scale_spiro, spiro_vol, label='Volume from respiratory flow', linewidth=2)
    line2, = ax.plot(scale_bellow, sig_rb_cut, label='Respiratory Bellow')
...
def plot_signal_curve():
    ...
    ax2.set_ylabel("Mean signal intensivity curve")
...
tk.Label(control_frame, text="Slice Number:").pack()
slice_entry = tk.Entry(control_frame)
slice_entry.insert(0, "0")
slice_entry.pack()

tk.Button(control_frame, text="Select respiratory bellow file", command=select_bauchgurt).pack(pady=2)
bauchgurt_label = tk.Label(control_frame, text="No file selected")
bauchgurt_label.pack()

tk.Button(control_frame, text="Select timestamp.npy", command=select_zeitstempel).pack(pady=2)
zeitstempel_label = tk.Label(control_frame, text="No file selected")
zeitstempel_label.pack()

tk.Button(control_frame, text="Select spiro.npy", command=select_spiro).pack(pady=2)
spiro_label = tk.Label(control_frame, text="No file selected")
spiro_label.pack()

tk.Button(control_frame, text="Select indizes.npy", command=select_indizes).pack(pady=2)
indizes_label = tk.Label(control_frame, text="No file selected")
indizes_label.pack()

tk.Button(control_frame, text="2. Look through slice!", command=start_analysis).pack(pady=5)

tk.Button(control_frame, text="1. Plot complete data!", command=plot_everything).pack(pady=5)
